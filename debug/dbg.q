...
cnst:(();0;1;,;`;::;::;';/;\;':;/:;\:);cnst[6]:mv; / basic consts
...
c:256#{(y+1;(::;::;f;f:{fr . x};get;::)d;(cnst;cnst@7+;6,;5,;x 3;4_x)[d:dm c]im c:z 0)}; / cnst0, adv, arg, loc, glob, const
...
c[1]:{z;(y;{ex:@[{'fr x};x;::];if[e;e-:1;fr::{$[(::)~y:y 3;'x;y]}[ex]/[{6<>x 2};fr];fr[2]:5;:app[fr cc 0;enlist ex]];'ex};y)}; / raise an exception, execute catch
...
c[10 82 83]:{(y-a;{$[104=type enlist . a:1_ z:fr z;z[0]. a;[fr[4]:cc 0;if[x;gt 1];appf2[y;z]]]}[10=z 0;83=z 0];y-til 1+a:1^z 1)}; / applications, enlist will deal with the magic value

/ helpers
gt:{fr[1]+:x}; / goto +N
cc:{fr[0;fr 1] x}; / current bytecode
ass:{0N!(x;y);fr[0N!cc 0]:$[4=count fr[0;3+fr 1];::;$[y~();get x;x . y]];gt 2}; / assign + implicit return = null value
...
empargs:{where 104=type each(,)./:1,/:x}; / idx of missing args
...
appN:({$[0>type y;@[x;y;:;z x y];.z.s[;;z]/[x;y]]}; / @,  3 args
   {[a;i;f;v]$[0>type i;@[y;i;:;x[a i;v]];.z.s[;;f]/[a;i;v]]}; / . 3 args
   {[i1;a;i2;f]$[0=count i2;.[a;i1;:;f a . i1];0>type i:i2 0;.z.s[i1;;;f]/[a;i,:\1_i2];.z.s[i1,i;;;f]/[a;1_i2]]}(); / @ 4 args
...
apm[101 102]:{$[x in (@;.);appf[x;y];x in (!;?);appq[x;y];x . y]}; / special forms/selects/other fns
apm[104]:{if[nargsp x,y;x . y];tpart[x;y]}; / parted fn
apm[105]:{app[{x y . z};value[x],y]}; / composite fn
...
/ adverbs
adv:6#(::);
adv[0]:{z;if[any f:99=t:type each y;if[not(all f)|all c~\:k:distinct raze c:key each y w:where f;'`domain];y[w]:y[w]@\:k;:k!.z.s[x;$[98=type y;value flip y;y];0]];
...
adv[1 2]:{s:({y};{x,enlist y})z=2;n:nargs x;c:count y;r:();if[(c=1)&n=1;f:l:y 0;while[1;r:s[r;l];if[(f~l)|(l:x l)~l;:r]]]; / iterate on val
  ...
  if[any f:99=t:type each a:(c:c<>1)_y;if[not all 1_~'[k 0;k:key each a w:where f];'`domain];$[98=type y;y:value flip y;y[c+w]:value each a w];:$[z=1;::;![k 0]adv[1][x;y;z]]]; / dict case
...
adv[3]:{if[99=type l:last y;y:(-1_y),enlist value l;:(key l)!adv[3][x;y;z]]; / dict case
...
adv[4 5]:{s:z=4;if[99=t:type y s;k:key y s;y[s]:value y s;:k!adv[4][x;y;z]];
...
gc:{.[;(where c<10;0);0 1!()]raze enlist[(),7]gb[x]\(where differ sums prev 0=0{$[x=0;0^0 0 0 1 2 1 1 2 1 1 1@y;x-1]}\c)cut c-(c=9)&7=c til[count c]-next c:-1_x 0}; / process fn to get its code
gf:{if[not(::)~f:fs x;:f];:fs[x]:(gc v;-1;5;::;::),(::;x,count[v 2]#enlist()),.z.P,#[last (v:-1_value x)0;::]}; / get fn frame
gif:{fr::@[gf x;3 5;:;(z;y)];sTop::8;pc::()}; / get inited fn frame (set parent)
...
i:{gif .$[100=type x;(x;(),y);({x . y};(x;(),y))],(::);gt 1;prs[]}; / set initial frame
s0:{if[fr 2;fr[2]:5;@[{if[tm<.z.P-sT;'"Dbg timeout"];i:fr 7;v:x[1]x 2;if[i=fr 7;fr[x 0]:v];if[(0<fr 1)&count x 0;sTop::x 0];gt 1};cc[];{fr[2 4]:(1;x)}]]}; / run 1 instruction
r:{i[x;y];cont[]}; / run until stop/exc/brk
s:{senv[];s0[];prs[]}; / one step
cont:{senv[];{4<fr 2}s0/0;prs[]}; / continue
l:{senv[];{(4<fr 2)&(pc::cc 0;(ns=(value fr[6;0])[3;0])|not()~pc)1}s0/0;prs[]}; / next line TODO: ignore some fns
